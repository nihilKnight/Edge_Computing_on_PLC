...
class PLCApp {
  ...
  initSocket() {
    ...
    this.socket.on("connectPLCResult", (result) => {
      if (result.success) {
        this.showMessage(
          `${this.getPLCName(result.plcId)} 连接成功`,
          "success"
        );
  ...
  initEventListeners() {
    ...
    // 添加预定义变量按钮
    document
      .getElementById("addPredefinedVariableBtn")
    ...
    // 刷新变量按钮
    document
      .getElementById("refreshVariablesBtn")
      .addEventListener("click", () => {
        if (this.currentPLC) {
    ...
    // 添加变量表单提交
    document
      .getElementById("addVariableForm")
  ...
  initModals() {
    // 关闭模态框事件
    document
      .querySelectorAll('.modal-close, .btn[id*="cancel"]')
      .forEach((btn) => {
  ...
  renderPLCList() {
    ...
    plcListContainer.innerHTML = this.plcList
      .map(
        (plc) => `
            <div class="plc-item ${plc.id === this.currentPLC ? "active" : ""}" 
                ...
                <div class="plc-variables-count">变量数量: ${
                  plc.variableCount
                }</div>
                <div class="plc-controls">
                    <button class="btn btn-small ${
                      plc.isConnected ? "btn-secondary" : "btn-success"
                    }" 
                            onclick="event.stopPropagation(); app.${
                              plc.isConnected ? "disconnectPLC" : "connectPLC"
                            }('${plc.id}')">
                        <i class="fas fa-${
                          plc.isConnected ? "unlink" : "plug"
                        }"></i>
                        ${plc.isConnected ? "断开" : "连接"}
                    </button>
                    <button class="btn btn-small btn-danger" 
                            onclick="event.stopPropagation(); app.deletePLC('${
                              plc.id
                            }')">
        ...
        return `
                 <div class="variable-item" data-node-id="${variable.nodeId}">
                     <div class="variable-header">
                         ...
                         <div class="variable-type ${dataType}">${
          variable.dataType
        }</div>
                     </div>
                     ...
                     ${
                       variable.browsePath
                         ? `<div class="variable-browse-path">路径: ${variable.browsePath}</div>`
                         : ""
                     }
                     <div class="variable-value">
                         <div class="variable-current-value" id="value-${this.escapeNodeId(
                           variable.nodeId
                         )}">
                             ${
                     ...
                     ${
                       variable.description
                         ? `<div style="font-size: 0.8rem; color: #64748b; margin-top: 0.5rem;">${variable.description}</div>`
                         : ""
                     }
                     ${
                       variable.error
                         ? `<div style="font-size: 0.8rem; color: #ef4444; margin-top: 0.5rem;">错误: ${variable.error}</div>`
                         : ""
                     }
                 </div>
             `;
  ...
  updateVariableValue(nodeId, value, dataType) {
    const valueElement = document.getElementById(
      `value-${this.escapeNodeId(nodeId)}`
    );
    if (valueElement) {
      // 添加更新动画效果
    ...
    if (this.variables[this.currentPLC]) {
      const variable = this.variables[this.currentPLC].find(
        (v) => v.nodeId === nodeId
      );
      if (variable) {
  ...
  detectEnvNodes(variableArray) {
    ...
    for (const v of variableArray) {
      ...
      if (name === "temperature" || name === "temp")
        sensors.temp.nodeMap.value = v.nodeId;
      if (name === "t_mean" || name === "temp_mean")
        sensors.temp.nodeMap.mean = v.nodeId;
      if (name === "t_variance" || name === "temp_variance")
        sensors.temp.nodeMap.variance = v.nodeId;
      if (name === "t_std_deviation" || name === "temp_std_deviation")
        sensors.temp.nodeMap.stdDev = v.nodeId;
      if (name === "t_z_score" || name === "temp_z_score")
        sensors.temp.nodeMap.zScore = v.nodeId;
      if (name === "t_anm_flag" || name === "temp_alarm")
        sensors.temp.nodeMap.alarm = v.nodeId;
      if (name === "t_freq" || name === "temp_freq")
        sensors.temp.nodeMap.freq = v.nodeId;
      if (name === "t_ampl" || name === "temp_ampl")
        sensors.temp.nodeMap.ampl = v.nodeId;
      if (name === "t_offset" || name === "temp_offset")
        sensors.temp.nodeMap.offset = v.nodeId;

      // Humidity 相关变量
      if (name === "humidity" || name === "hum")
        sensors.hum.nodeMap.value = v.nodeId;
      if (name === "h_mean" || name === "hum_mean")
        sensors.hum.nodeMap.mean = v.nodeId;
      if (name === "h_variance" || name === "hum_variance")
        sensors.hum.nodeMap.variance = v.nodeId;
      if (name === "h_std_deviation" || name === "hum_std_deviation")
        sensors.hum.nodeMap.stdDev = v.nodeId;
      if (name === "h_z_score" || name === "hum_z_score")
        sensors.hum.nodeMap.zScore = v.nodeId;
      if (name === "h_anm_flag" || name === "hum_alarm")
        sensors.hum.nodeMap.alarm = v.nodeId;
      if (name === "h_freq" || name === "hum_freq")
        sensors.hum.nodeMap.freq = v.nodeId;
      if (name === "h_ampl" || name === "hum_ampl")
        sensors.hum.nodeMap.ampl = v.nodeId;
      if (name === "h_offset" || name === "hum_offset")
        sensors.hum.nodeMap.offset = v.nodeId;

      // Pressure 相关变量
      if (name === "pressure" || name === "press")
        sensors.press.nodeMap.value = v.nodeId;
      if (name === "p_mean" || name === "press_mean")
        sensors.press.nodeMap.mean = v.nodeId;
      if (name === "p_variance" || name === "press_variance")
        sensors.press.nodeMap.variance = v.nodeId;
      if (name === "p_std_deviation" || name === "press_std_deviation")
        sensors.press.nodeMap.stdDev = v.nodeId;
      if (name === "p_z_score" || name === "press_z_score")
        sensors.press.nodeMap.zScore = v.nodeId;
      if (name === "p_anm_flag" || name === "press_alarm")
        sensors.press.nodeMap.alarm = v.nodeId;
      if (name === "p_freq" || name === "press_freq")
        sensors.press.nodeMap.freq = v.nodeId;
      if (name === "p_ampl" || name === "press_ampl")
        sensors.press.nodeMap.ampl = v.nodeId;
      if (name === "p_offset" || name === "press_offset")
        sensors.press.nodeMap.offset = v.nodeId;
    }
  }
  ...
  updateSensorData(sensorKey, lookup) {
    ...
    const phase = nodeMap.phase ? getValue(nodeMap.phase) : 0;
    ...
    this.renderSensorSineChart(sensorKey, freq, ampl, offset, phase);
  }
  ...
  updateSensorDataFromMap(sensorKey, variablesMap) {
    ...
    const ampl = nodeMap.ampl ? getValue(nodeMap.ampl) : null;
    const offset = nodeMap.offset ? getValue(nodeMap.offset) : null;
    const phase = nodeMap.phase ? getValue(nodeMap.phase) : null;

    // 更新折线图数据
    if (value != null) {
      const timestamp = nodeMap.value
        ? variablesMap[nodeMap.value]?.timestamp
        : null;
  ...
  // 渲染传感器正弦波图
  renderSensorSineChart(sensorKey, freq, ampl, offset, phase) {
    const sensor = this.env.sensors[sensorKey];
    ...
    if (!sensor._sineStartTimeMs) sensor._sineStartTimeMs = Date.now();
    const nowMs = Date.now();
    const startMs = nowMs - PAST_MS;
    const endMs = nowMs + FUTURE_MS;
    const dtMs = TOTAL_MS / points;
    ...
    for (let i = 0; i <= points; i++) {
      ...
      if (tMs <= nowMs) {
        ...
        lineFuture.push([tMs, Number(y.toFixed(3))]);
      }
    }

    const scatterData = sensor.lineData
      .filter((d) => d.time >= startMs)
      .map((d) => [d.time, Number(d.value.toFixed(3))]);
    ...
    sensor.sineChart.setOption({
      ...
      legend: { data: ["预测(已发生)", "预测(未来)", "实时"] },
      markLine: {
        data: [{ xAxis: nowMs, name: "当前" }],
    ...
    if (formulaEl) {
      const a = Number(ampl).toFixed(3);
      const f = Number(freq).toFixed(4);
      const b = Number(offset).toFixed(3);
      formulaEl.textContent = `y = ${a} · sin(2π·${f}·t) + ${b}`;
    }
  }
  ...
  getDataTypeCategory(dataType) {
    ...
    if (
      ["Int16", "Int32", "UInt16", "UInt32", "Float", "Double"].includes(
        dataType
      )
    )
      return "Number";
